...
export class ConfigService {
  ...
  id: number = 1;
  constructor(
    @InjectRepository(Config) private repository: Repository<Config>,
    @InjectRepository(ConfigEcommerce)
    private repositoryConfige: Repository<ConfigEcommerce>,
  ) {}
  ...
  async findOne() {
    ...
    if (value == 0) {
      throw new HttpException(
        'No existe una configuración de tienda creada',
        HttpStatus.CONFLICT,
      );
    }
  ...
  async updateEmailConfig(emailconfig: EmailConfigDto) {
    return (
      await this.repository.update(this.id, {
        email_notificaciones: emailconfig.email_notificaciones,
      })
    ).affected;
  }
  ...
  async getServerEmailConfig() {
    return this.repository.findOne(this.id, {
      select: ['email_host', 'email_puerto'],
    });
  }
  ...
  async getTemplatesQuote() {
    return this.repository.findOne(this.id, {
      select: [
        'plantilla_cotizacion_encabezado_pagina',
        'plantilla_cotizacion_terminos_pago',
        'plantilla_cotizacion_condiciones',
        'plantilla_cotizacion_pie_pagina',
        'plantilla_cotizacion_garantia',
        'plantilla_cotizacion_email_mensaje',
  ...
  /**
   * Obtener el nit emisor de configuracion-fel
   *
   */
  async getNitEmisor() {
    return this.repository.query('Select nit_emisor from configuracion_fel');
  }
}
