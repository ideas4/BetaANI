...
export class InventoryController {
  ...
  @UseGuards(AuthGuard('jwt'))
  @ApiOperation({
    summary: 'Obtiene el registro de la información del inventario',
  })
  @ApiOkResponse({ status: 200, description: 'Inventario Ok' })
  @ApiForbiddenResponse({ description: 'No está autorizado' })
  ...
  @UseGuards(AuthGuard('jwt'))
  @ApiOperation({
    summary: 'Obtiene la información del inventario resultado de una búsqueda',
  })
  @ApiOkResponse({ status: 200, description: 'Inventario Ok' })
  @ApiForbiddenResponse({ description: 'No está autorizado' })
  ...
  @ApiOperation({
    summary:
      'Obtener un producto y verificar existencia en todas las sucursales Para agregar a cotización',
  })
  @ApiOkResponse({ status: 200, description: 'Inventario Ok' })
  @ApiForbiddenResponse({ description: 'No está autorizado' })
  ...
  @ApiOkResponse({ status: 200, description: 'Inventario Ok' })
  @ApiForbiddenResponse({ description: 'No está autorizado' })
  findToAddOrderFill(@Param('id') id: string) {
    return this.inventoryService.findToFillProduct(+id);
  }

  /**
   * Obtener de un producto segun su inventario y sucursal
   */
  @Get(':id/:idSucursal')
  @ApiBearerAuth()
  @UseGuards(AuthGuard('jwt'))
  @ApiOperation({
    summary: 'Obtiene el registro de inventario segun su sucursal.',
  })
  @ApiOkResponse({ status: 200, description: 'Inventario Ok' })
  @ApiForbiddenResponse({ description: 'No está autorizado' })
  findToAddProductOrderFill(
    @Param('id') id: string,
    @Param('idSucursal') idSucursal: number,
  ) {
    return this.inventoryService.findToFillProductSucursal(+id, idSucursal);
  }
  ...
  @UseGuards(AuthGuard('jwt'))
  @ApiOperation({
    summary: 'Agregar producto dañado a una entrada de inventario',
  })
  @ApiOkResponse({ status: 200, description: 'Inventario Ok' })
  @ApiForbiddenResponse({ description: 'No está autorizado' })
  ...
  @UseGuards(AuthGuard('jwt'))
  @ApiOperation({
    summary: 'Mover unidades de inventario de una sucursal a otra',
  })
  @ApiOkResponse({ status: 200, description: 'Inventario Ok' })
  @ApiForbiddenResponse({ description: 'No está autorizado' })
